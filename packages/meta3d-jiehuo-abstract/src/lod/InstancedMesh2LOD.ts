...
import { getWorldPosition } from "../utils/TransformUtils"
...
export let create = (state: state, levels: Array<InstanceSourceLOD.level>, count: number, config: InstancedMesh2Params<any>): [state, InstancedMesh2LOD] => {
    let instancedMesh2Levels = levels.sort((a, b) => {
        ...
        instancedMesh2Levels.push({
            instancedMesh2s: InstanceSourceLOD.getLevelMeshes(level).reduce((instancedMesh2s, { geometry, material, castShadow, receiveShadow }) => {
                let instancedMesh2 = new InstancedMesh2(geometry, material as Material, count, config)
                instancedMesh2.castShadow = castShadow
                instancedMesh2.receiveShadow = receiveShadow

                Object3DUtils.markNotNeedsUpdate(instancedMesh2)

                instancedMesh2s.push(instancedMesh2)
...
export let updateCulling = (state: state, mesh: InstancedMesh2LOD, camera: Camera) => {
    ...
    findAllSources<InstanceSourceLOD.InstanceSourceLOD>(state, mesh.id).forEach(sourceLOD => {
        ...
        worldPosition = getWorldPosition(sourceLOD, worldPosition)
...
export let setMatrixWorld = (state: state, mesh: InstancedMesh2LOD, index: number, sourceLOD: InstanceSourceLOD.InstanceSourceLOD) => {
    _getInstancedMesh2Levels(state, mesh.id).forEach(({ instancedMesh2s }, levelIndex) => {
        instancedMesh2s.forEach((instancedMesh2, childIndex) => {
            instancedMesh2.setMatrixWorld(index, sourceLOD.getLevelChild(levelIndex, childIndex).matrixWorld)
        })
    })
}
