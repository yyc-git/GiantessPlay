...
let _getState = (state: state): bulletHitEmitter => {
	return getExn(getParticleState(state).propBulletHitEmitter)
}
...
let _setState = (state: state, bulletHitEmitterState: bulletHitEmitter) => {
	return {
		...
		particle: {
			...
			propBulletHitEmitter: return_(bulletHitEmitterState)
		}
	}
}
...
export let init = (state: state, scene) => {
	...
	}, scene, "meta3d-jiehuo-abstract/particle/propBulletHit.png", "#ffffff",
		false
	)
}
