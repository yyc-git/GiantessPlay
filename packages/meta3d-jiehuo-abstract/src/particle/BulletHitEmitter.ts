import { bulletHitEmitter, bulletHitEmitterParam, bulletHitParticle, particle, state } from "../type/StateType"
...
let _getState = (state: state): bulletHitEmitter => {
	return getExn(getParticleState(state).bulletHitEmitter)
}

let _setState = (state: state, bulletHitEmitterState: bulletHitEmitter) => {
	return {
		...state,
		particle: {
			...getParticleState(state),
			bulletHitEmitter: return_(bulletHitEmitterState)
		}
	}
}

export let createState = (): bulletHitEmitter => {
	return {
		geometry: getEmpty(),
		particles: []
	}
...
export let init = (state: state, scene) => {
	...
	}, scene, "meta3d-jiehuo-abstract/particle/bulletHit.png", "#ffffff",
		true
	)
}
