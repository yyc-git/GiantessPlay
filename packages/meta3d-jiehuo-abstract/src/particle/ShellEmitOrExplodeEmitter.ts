import { shellEmitOrExplodeEmitter, shellEmitOrExplodeEmitterParam, shellEmitOrExplodeParticle, particle, state } from "../type/StateType"
...
let _getState = (state: state): shellEmitOrExplodeEmitter => {
	return getExn(getParticleState(state).shellEmitOrExplodeEmitter)
}

let _setState = (state: state, shellEmitOrExplodeEmitterState: shellEmitOrExplodeEmitter) => {
	return {
		...state,
		particle: {
			...getParticleState(state),
			shellEmitOrExplodeEmitter: return_(shellEmitOrExplodeEmitterState)
		}
	}
}

export let createState = (): shellEmitOrExplodeEmitter => {
	return {
		geometry: getEmpty(),
		particles: []
	}
...
export let init = (state: state, scene) => {
	...
	}, scene, "meta3d-jiehuo-abstract/particle/shellEmitOrExplode.png", "#ffffff",
...
export let emit = (state: state, {
	...
	size,
	position,
}: shellEmitOrExplodeEmitterParam) => {
