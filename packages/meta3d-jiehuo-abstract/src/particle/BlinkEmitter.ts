import { blinkEmitter, blinkEmitterParam, state } from "../type/StateType"
...
let _getState = (state: state): blinkEmitter => {
	return getExn(getParticleState(state).blinkEmitter)
}

let _setState = (state: state, blinkEmitterState: blinkEmitter) => {
	return {
		...state,
		particle: {
			...getParticleState(state),
			blinkEmitter: return_(blinkEmitterState)
		}
	}
}

export let createState = (): blinkEmitter => {
	return {
		geometry: getEmpty(),
		particles: []
	}
...
export let init = (state: state, scene) => {
	...
	}, scene, "meta3d-jiehuo-abstract/particle/blink.png", "#ffffff",
		false
	)
}
