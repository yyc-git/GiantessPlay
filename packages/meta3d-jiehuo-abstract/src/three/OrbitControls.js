...
class OrbitControls extends EventDispatcher {
	...
	constructor(object, domElement) {
		...
		/*!edit by meta3d */
		this.isEnablePointerlock = false
		this._lastExitPointerLockTime = null
		...
		this.pointerlockchangeHandler = () => {
			if (document.pointerLockElement == scope.domElement) {
				document.addEventListener('mousemove', scope._handleMouseMoveRotate, false);
				document.addEventListener('wheel', scope._onMouseWheel, { passive: false });
				...
				scope.isEnablePointerlock = false
				scope._lastExitPointerLockTime = performance.now()

				document.removeEventListener('mousemove', scope._handleMouseMoveRotate);
				document.removeEventListener('wheel', scope._onMouseWheel);

				state = STATE.NONE;
			}
		}
		// this.lock = () => {
		// 	// if (this.isEnablePointerlock) {
		// 	// 	return
		// 	// }
		...
		// 	// */
		// 	// if (this._lastExitPointerLockTime !== null
		// 	// 	&& performance.now() - this._lastExitPointerLockTime < 1500) {
		// 	// 	return
		// 	// }

		// 	// scope.domElement.requestPointerLock()

		// 	this.isEnablePointerlock = true
		// }
		// this.unlock = () => {
		...
		function handleTouchStartRotate(event) {
			...
			if (pointers.length === 1) {
				...
				rotateStart.set(getPageX(event), getPageY(event));

			} else {

				const position = getSecondPointerPosition(event);

				const x = 0.5 * (getPageX(event) + position.x);
				const y = 0.5 * (getPageY(event) + position.y);
		...
		function handleTouchStartPan(event) {
			...
			if (pointers.length === 1) {
				...
				panStart.set(getPageX(event), getPageY(event));
		...
		function handleTouchStartDolly(event) {
			...
			const dx = getPageX(event) - position.x;
			const dy = getPageY(event) - position.y;
		...
		function handleTouchMoveRotate(event) {
			...
			if (pointers.length == 1) {
				...
				rotateEnd.set(getPageX(event), getPageY(event));
		...
		function handleTouchMovePan(event) {
			...
			if (pointers.length === 1) {
				...
				panEnd.set(getPageX(event), getPageY(event));
		...
		function handleTouchMoveDolly(event) {
			...
			const centerX = (getPageX(event) + position.x) * 0.5;
			const centerY = (getPageY(event) + position.y) * 0.5;
		...
		function trackPointer(event) {
			...
			position.set(getPageX(event), getPageY(event));
