...
let _waitForFinish = (state: state) => {
    ...
    if (
        ...
        return markFinish(state, getIdExn(state, actionName.TwoHands), behaviourTreeNodeExecuteResult.Success, getKey())
    }
...
export let keepTwoHandsOneFinger: actionNodeFunc = (state, id) => {
    Console.log("keepTwoHandsOneFinger")

    return triggerAction(state, actionName.KeepTwoHandsOneFinger as any).then(([state, result_]) => {
        if (result_ == result.Fail) {
            return Promise.resolve([state, behaviourTreeNodeExecuteResult.Fail])
        }

        state = setId(state, actionName.TwoHands, id)
