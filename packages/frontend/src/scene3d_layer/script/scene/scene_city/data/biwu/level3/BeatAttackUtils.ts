...
import { getGirlVolume, getLittleManVolume } from "../../../utils/SoundUtils"
import { getBeatSoundResourceId } from "../../../manage/biwu/level3/ManageScene"
import { getGirlState } from "../../../girl/Girl"
import { getOrbitControlsTarget } from "../../../LittleManCamera"
...
let _buildKeyForDeferByTime = () => "BeatAttack_DeferByTime"
...
let _downAndUp = (state,
    ...
    switch (collisionPart_) {
        case collisionPart.RightHand:
            downTarget = getDownTarget(state, worldPosition, 5, "右中指１", "右腕ＩＫ")
            break
        case collisionPart.LeftHand:
            downTarget = getDownTarget(state, worldPosition, 5, "左中指１", "左腕ＩＫ")
            break
        default:
            throw new Error("err")
    }
        ...
        [
            ...
            state => {
                ...
                return computeDamage(state, collisionPartOBB, buildDownDirection(), _getRandomForce(force)).then(([state, isHit]) => {
                    getGirlState(state).screenShake.shake(getOrbitControlsTarget, 3, 150)

                    state = setAbstractState(state, SoundManager.addNeedToPlaySound(getAbstractState(state), SoundManager.buildNeedToPlaySoundData(getBeatSoundResourceId(), getIsDebug(state), getGirlVolume(state, LittleManTransform.getWorldPosition(state)))))
