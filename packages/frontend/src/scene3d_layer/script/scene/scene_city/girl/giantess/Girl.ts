...
import { getThirdPersonCameraTarget, getCameraType, getOrbitControlsTarget, isCameraCollision, updateFirstPersonControls, updateThirdPersonControlsForChangeScale } from "../../Camera"
...
import { getAllAnimationNames, getCurrentAnimationName, isChangeScaling, isCompletelyPlayingAnimation, isKeepPoseAnimation, isPlayingAnimation, isPlayingAnimationByWeight, playBiggerAnimation, updateAnimation, updateAnimationDuration } from "../Animation"
...
import { buildGiantessStatusUpdateEventNameEventData, getGiantessStatusUpdateEventName, getLevelStatusUpdateEventName } from "../../../../../utils/EventUtils"
import { actionName, animationName, articluatedAnimationName, effect, excitement, forceSize, frameIndex, particle, track } from "../../data/DataType"
import { damageType, collisionPart, girl, stompPhase, walkPhase, soundType, force, objectStateName, thirdPersonCameraTarget, pose, actionState, scaleState, result, giantessStrength, giantessScale, excitementIncrease } from "../../type/StateType"
...
export let update = (state: state) => {
    ...
    return updateAnimation(state).then(state => {
        ...
        if (getCameraType(state) == cameraType.FirstPerson) {
            state = updateFirstPersonControls(state)
        }
        else {
        ...
        if (newHeight != oldHeight) {
            ...
            if (getCameraType(state) == cameraType.ThirdPerson) {
                // if (isGiantessRoad(state)) {
...
export let isStressingByRate = (state: state, damage, weaponType_: weaponType, collisionPart_: collisionPart, damagePart) => {
