...
import { getScene, getBasicBulletGunEmmitSoundResourceId, getState as getCitySceneState, setState as setCitySceneState, getName as getCitySceneName, getName, getRocketGunEmmitSoundResourceId } from "../../../CityScene"
...
export let getNamePrefix = () => `${Soldier.getNamePrefix()}_rocketeer`
...
export let isRocketeer = (name: string) => {
...
export let initWhenImportScene = (state: state) => {
    let { resourceId, resourcePath } = getModelData(state, modelName.Rocketeer)
    ...
    return ModelLoader.parseFbx(getAbstractState(state), Loader.getResource(getAbstractState(state), resourceId), resourcePath,).then(rocketeer => {
        return ModelLoader.parseFbx(getAbstractState(state), Loader.getResource(getAbstractState(state), animationName.Idle), getIdleAnimationResourcePath(getCitySceneName())).then(idle => {
            return ModelLoader.parseFbx(getAbstractState(state), Loader.getResource(getAbstractState(state), animationName.Run), getRunningAnimationResourcePath(getCitySceneName())).then(run => {
                return ModelLoader.parseFbx(getAbstractState(state), Loader.getResource(getAbstractState(state), animationName.Emit), getEmitAnimationResourcePath(getCitySceneName())).then(emit => {
                    // return ModelLoader.parseFbx(getAbstractState(state), Loader.getResource(getAbstractState(state), animationName.Emit), getShootingAnimationResourcePath(getCitySceneName())).then(emit => {
                        ...
                        return ModelLoader.parseFbx(getAbstractState(state), Loader.getResource(getAbstractState(state), animationName.Death), getDeathAnimationResourcePath(getCitySceneName())).then(death => {
                            return ModelLoader.parseFbx(getAbstractState(state), Loader.getResource(getAbstractState(state), animationName.Controlled), getControlledAnimationResourcePath(getCitySceneName())).then(controlled => {
                                ...
                                state = Soldier.setState(state, {
                                    ...
                                    initialQuaternionMap: Soldier.getState(state).initialQuaternionMap.set(modelName.Rocketeer,
                                ...
                                state = setBonePositionOffset(state, rocketeer, getModelData(state, modelName.Rocketeer).positionOffset)
...
let _getEmitClipIndex = (name) => {
    return animationClipIndex.Emit
}
...
let _getBulletStartLocalPosition = (soldierScale) => {
    ...
    // return new Vector3(-0.915845, 10, 4.49601).multiply(soldierScale)
    return new Vector3(-0.915845, 10, 4.49601 + 5).multiply(soldierScale)
}
...
export let createShootState = (): fsm_state<state> => {
    return {
        ...
        enterFunc: (state) => Promise.resolve(state),
        executeFunc: (state, name) => {
            let clipIndex = _getEmitClipIndex(name)
                ...
                (state,
                    ...
                    return setAbstractState(state, Flow.addDeferExecFuncDataByTime(getAbstractState(state), (state) => {
                        state = setAbstractState(state, SoundManager.addNeedToPlaySound(getAbstractState(state),
                            SoundManager.buildNeedToPlaySoundData(getRocketGunEmmitSoundResourceId(), getIsDebug(state), getVolume(state, emitVolume, soldierPosition, 0))
                        ))

                        state = setAbstractState(state, ParticleManager.emitRocket(getAbstractState(state), {
                            fromName: name,
                            speed: emitterSpeed,
                            life: emitterLife,
                            size: emitterSize,
                        ...
                        return Promise.resolve(state)
                    }, computeTimeSecond(getEmitWorkFrameCount()), name))
                },


                getValue(state),
                RocketGun.getValue(state),
                Soldier.getInitialQuaternion(state, modelName.Rocketeer),
...
export let getPickedTransform = Soldier.getPickedTransform(modelName.Rocketeer)
