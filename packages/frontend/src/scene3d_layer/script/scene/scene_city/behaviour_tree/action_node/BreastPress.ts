...
import { judgeAttackRangeAndTriggerAction, lookatTarget, triggerAction } from "./Utils"
import { Console } from "meta3d-jiehuo-abstract"

export let breastPress: actionNodeFunc = (state, id) => {
    Console.log("breastPress")

    // if (getTarget(state).type == targetType.Building) {
    ...
    switch (getCurrentPose(state)) {
        ...
        case pose.Pick:
            return Promise.resolve([state, behaviourTreeNodeExecuteResult.Fail])
        case pose.Crawl:
            actionName_ = actionName.BreastPress
            break
    }

    state = lookatTarget(state)

    // return judgeAttackRangeAndTriggerAction(state, actionName_, id, attackRange.Middle)
